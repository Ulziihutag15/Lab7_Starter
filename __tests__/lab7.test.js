describe("Basic user flow for Website", () => {
  ...
  it("Make sure <product-item> elements are populated", async () => {
    ...
    prodItemsData.forEach((prodItem) => {
      if (prodItem.title.length == 0) {
        allArePopulated = false;
      }
      if (prodItem.price.length == 0) {
        allArePopulated = false;
      }
      if (prodItem.image.length == 0) {
        allArePopulated = false;
      }
    });
  ...
  it("Checking number of items in cart on screen", async () => {
    ...
    for (const prodItem of prodItems) {
      ...
      const text = await innerTextHandle.jsonValue();
      if (text === "Add to Cart") {
        await button.click();
      }
    }

    let cartCount = await page.$eval("#cart-count", (el) => el.innerText);
